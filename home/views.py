...
import pdfkit
from ai_chatbot.settings import pdfkit_config
...
def chatapi(request):
    if request.method=='POST':
        ...
        pdfkit.from_string(response.choices[0].text, 'response.pdf',configuration=pdfkit_config)
        ...
        return render(request,'chatbot.html',{'data':data,"user_input":user_input,"chatbot_response":chatbot_response})
    else:
...
def response_pdf(request):
    filepath=os.path.join('','response.pdf')
